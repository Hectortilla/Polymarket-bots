"""Explicit runtime capability gate, checked before any hook can execute."""

from polybot_control_plane.catalog.graphs.values import GraphOperation

from polybot_control_plane.catalog.node_based.evaluator.pure import PURE_OPERATIONS
from polybot_control_plane.catalog.node_based.evaluator.context_operations import (
    PORTFOLIO_OPERATIONS,
    DIAGNOSTIC_OPERATIONS,
)
from polybot_control_plane.catalog.node_based.evaluator.event_controls import (
    EVENT_CONTROL_OPERATIONS,
)

EXECUTABLE_OPERATIONS: frozenset[GraphOperation] = (
    PURE_OPERATIONS
    | PORTFOLIO_OPERATIONS
    | DIAGNOSTIC_OPERATIONS
    | EVENT_CONTROL_OPERATIONS
)


class UnsupportedGraphOperation(ValueError):
    pass
