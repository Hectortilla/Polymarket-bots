"""Pure signed-position transitions shared by execution and tracking."""
...
def _validate_transition_inputs(
    *,
    ...
    if not current_size.is_finite():
        raise ValueError("position size must be finite")
    if not fill_size.is_finite() or fill_size <= ZERO_POSITION_SIZE:
        raise ValueError("filled size must be positive and finite")
    if not fill_price.is_finite() or fill_price <= ZERO_POSITION_SIZE:
    ...
    if current_average_basis is not None and (
        not current_average_basis.is_finite()
