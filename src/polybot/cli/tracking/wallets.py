...
async def synchronize_followed_wallets(
    ...
    for wallet in new_wallets:
        await _bootstrap_followed_wallet(
            wallet,
    ...
    await _register_persisted_followed_markets(
        followed_wallets,
        gamma,
        registry,
    )
...
async def _bootstrap_followed_wallet(
    wallet: str,
    *,
    ...
    for position in positions:
        ...
        if market is None or not MarketIdentity.from_position(position).matches(market):
            raise RuntimeError("current wallet position has unresolved market identity")
...
async def _register_persisted_followed_markets(
    followed_wallets: FollowedWalletStore,
    gamma: MarketResolver,
    registry: TrackedMarketRegistry,
) -> None:
    """Re-register markets that persisted followed-wallet positions reference."""
