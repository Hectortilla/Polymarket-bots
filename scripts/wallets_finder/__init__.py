...
def scan_market(
    ...
    try:
        ...
        print(bad(f"  Failed to fetch market positions: {exc}"))
        return 0
    processed = load_seen_wallets(GOOD_FILE) | load_seen_wallets(BAD_FILE)
    holders = unique_holders(positions)
    completed = 0
    for wallet, holder_position in holders:
        ...
        try:
            ...
            print(warn(f"error {wallet}: {exc} (skipped)"))
            continue
        ...
        print(
            f"{wallet} size={size:,.0f} net={float(metrics['net_cash']):+,.2f} "
