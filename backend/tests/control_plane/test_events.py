...
def test_event_store_append_returns_a_persisted_event_without_mutating_input() -> None:
    ...
    def assign_id() -> None:
        session.add.call_args.args[0].id = FIRST_DURABLE_EVENT_ID

    session.flush.side_effect = assign_id

    stored = asyncio.run(EventStore(session).append(event))

    assert isinstance(stored, PersistedRunLifecycleEvent)
    assert stored.id == FIRST_DURABLE_EVENT_ID
