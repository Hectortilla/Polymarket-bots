...
broker.on_event(TaskiqEvents.WORKER_STARTUP)(start_worker)
broker.on_event(TaskiqEvents.WORKER_SHUTDOWN)(stop_worker)

TASK_CONTEXT_DEPENDENCY = TaskiqDepends()


@broker.task
async def execute_run_task(
    run_id: str, context: Context = TASK_CONTEXT_DEPENDENCY
) -> None:
    await execute_run(UUID(run_id), resources=worker_resources(context.state))
