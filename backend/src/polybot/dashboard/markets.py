"""Market and portfolio state shared by dashboard renderers."""
...
from .contracts import format_market_label, format_token_label
...
class DashboardMarkets:
    ...
    def current_book(self, token_id: str, now_ms: int) -> BookSnapshot | None:
        book = self.books.get(token_id)
        if book is None or self.book_max_age_ms is None:
            return book
        return book if book.is_fresh(now_ms, self.book_max_age_ms) else None
    ...
    def refresh_fill_mark(self, fill: FillEvent) -> None:
        ...
        if book is None:
            # Fill telemetry arrives before the corresponding dispatch completes.
