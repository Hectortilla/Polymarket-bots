...
from polybot.backtesting.scheduler.pending import PendingReplayEvents
...
class ReplayScheduler:
    def __init__(
        ...
        *,
        ...
        self._pending = PendingReplayEvents()
    ...
    async def _advance_during_callback(self, target_ms: int) -> None:
        ...
        while True:
            ...
            next_event_observed_at_ms = None if event is None else event.observed_at_ms
            next_time = min(
                target_ms,
                self._next_plan_refresh_ms,
                target_ms
                if next_event_observed_at_ms is None
                else next_event_observed_at_ms,
            )
            if next_time > self._clock.now_ms():
                await self._move_to(next_time)
            handled = False
            if (
                next_event_observed_at_ms is not None
                and next_event_observed_at_ms <= target_ms
                and next_event_observed_at_ms <= self._next_plan_refresh_ms
            ):
                replayed_event = await self._cursor.pop()
            ...
            if self._next_plan_refresh_ms <= target_ms and (
                ...
                or self._next_plan_refresh_ms < next_event_observed_at_ms
            ):
                await self._move_to(self._next_plan_refresh_ms)
                self._next_plan_refresh_ms += STREAM_PLAN_REFRESH_INTERVAL_MS
    ...
    def _apply_event_mutation(
        ...
        if resolution_was_queued and resolution is not None:
            self._pending.enqueue_resolution(resolution)
    ...
    async def _enqueue_bootstraps(self, new_slugs: set[str]) -> set[str]:
        ...
        for book in bootstraps:
            self._artifacts.record_book(book)
        self._pending.enqueue_books(bootstraps)
        return {book.token_id for book in bootstraps}

    async def _drain_pending(self) -> None:
        while (event := self._pending.pop()) is not None:
            if isinstance(event, BookSnapshot):
                await self._remember_outcome(await self._runner.dispatch_book(event))
            else:
    ...
    def _activate_blackouts_through(self, boundary_ms: int) -> None:
        ...
        self._pending.invalidate_books(invalidated_token_ids)
