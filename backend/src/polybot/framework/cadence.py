...
STREAM_PLAN_REFRESH_INTERVAL_SECONDS: Final = (
    STREAM_PLAN_REFRESH_INTERVAL_MS / MILLISECONDS_PER_SECOND
)
RESOLUTION_RECONCILIATION_SECONDS: Final = 30.0


def advance_deadline_past(
    deadline_seconds: float, interval_seconds: float, now_seconds: float
) -> float:
    """Advance a recurring deadline until it is strictly later than now."""
    while deadline_seconds <= now_seconds:
        deadline_seconds += interval_seconds
