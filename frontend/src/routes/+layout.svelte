<script lang="ts">
  ...
  import { isPublicInformationPath, PUBLIC_INFORMATION_PATH, PUBLIC_INFORMATION_LABEL, PUBLIC_PRELOAD_DATA_POLICY } from '$lib/public/navigation';
  ...
  onMount(() => {
    ...
    const interval = setInterval(() => { if ($account && !isPublicInformationPath(page.url.pathname)) void accountSession.restore(); }, runtimeContract.auth.sessionRecheckMs);
  ...
  $effect(() => {
    if (!isPublicInformationPath(page.url.pathname)) untrack(() => { void accountSession.restore(); });
  });
...
<header class="site-header" data-sveltekit-preload-data={PUBLIC_PRELOAD_DATA_POLICY}>
  <div class="site-header-inner">
    <a class="brand" href={isPublicInformationPath(page.url.pathname) ? PUBLIC_INFORMATION_PATH.WELCOME : NAVIGATION_PATH.HOME} aria-label={SERVICE_IDENTITY.name}>
    ...
    {#if isPublicInformationPath(page.url.pathname)}
      <nav class="account-controls" aria-label="Preview navigation"><a href={PUBLIC_INFORMATION_PATH.HELP}>{PUBLIC_INFORMATION_LABEL[PUBLIC_INFORMATION_PATH.HELP]}</a><a href={PUBLIC_INFORMATION_PATH.SUPPORT}>{PUBLIC_INFORMATION_LABEL[PUBLIC_INFORMATION_PATH.SUPPORT]}</a><a href={LOGIN_PATH}>{AUTH_COPY.SIGN_IN}</a></nav>
    {:else if $account}
      <div class="account-controls"><span>{$account.email}</span><a href={PUBLIC_INFORMATION_PATH.HELP}>{PUBLIC_INFORMATION_LABEL[PUBLIC_INFORMATION_PATH.HELP]}</a><a href={PUBLIC_INFORMATION_PATH.SUPPORT}>{PUBLIC_INFORMATION_LABEL[PUBLIC_INFORMATION_PATH.SUPPORT]}</a><a href={runtimeContract.accountManagement.accountPath}>{ACCOUNT_COPY.SETTINGS}</a><button onclick={() => void accountSession.signOut()}>{AUTH_COPY.SIGN_OUT}</button></div>
    {/if}
  </div>
</header>
