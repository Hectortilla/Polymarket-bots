...
describe("node graph editor", () => {
  ...
  it("constructs real Flow nodes and remounts the emitted graph", async () => {
    ...
    const comparisonNode = emittedGraph.nodes.find((node) => node.type === GRAPH_NODE_TYPE.comparison);
    ...
    const sourceHandle = document.querySelector<HTMLElement>(
      `[data-nodeid="${constantNode.id}"]` + `[data-handleid="${NUMBER_CONSTANT.output.handle_id}"]`,
    );
    const targetHandle = document.querySelector<HTMLElement>(
      `[data-nodeid="${comparisonNode.id}"]` + `[data-handleid="${EQUAL_COMPARISON.inputs[0].handle_id}"]`,
    );
  ...
  it("removes only an expandable input edge and explains incompatible parameter wiring", async () => {
    stubFlowBrowserApis();
    const graph = parityContract.cases.find((item) => item.name === "Multiple conditions and cooldown")!
      .graph as NodeGraph;
    const onchange = vi.fn<(graph: NodeGraph) => void>();
    render(NodeGraphInput, {
      initialGraph: graph,
    ...
    expect(edited.edges).toEqual(
      graph.edges!.filter((edge) => !(edge.target === "conditions" && edge.target_handle === "input_2")),
    );
    ...
    expect(
      screen.getAllByRole("status").some((element) => element.textContent?.includes("incompatible connection")),
    ).toBe(true);
  });
