...
import { constantDataFromDescriptor, constantDataFromInput, constantInput } from "./constantNode";
...
describe("constant node inputs", () => {
  ...
  it.each([
    ...
    expect(constantDataFromDescriptor(descriptor(scalarType, value))).toEqual({ scalar_type: scalarType, value });
  });

  it("rejects a catalog default that disagrees with its scalar type", () => {
    expect(() => constantDataFromDescriptor(descriptor(GRAPH_SCALAR_TYPE.boolean, "true"))).toThrow(
      "Invalid boolean constant default",
    );
  });
});
