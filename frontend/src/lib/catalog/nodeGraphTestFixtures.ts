...
function requireDescriptor<T>(descriptors: T[], matches: (descriptor: T) => boolean, description: string): T {
...
export const LESS_THAN_OR_EQUAL: GraphComparisonDescriptor = requireDescriptor(
  ...
  (descriptor) => descriptor.operator === catalogContract.graphComparisonOperator.LESS_THAN_OR_EQUAL,
  "less-than-or-equal comparison",
);
export const EQUAL_COMPARISON: GraphComparisonDescriptor = requireDescriptor(
  TEST_GRAPH_CATALOG.comparisons,
  (descriptor) => descriptor.operator === catalogContract.graphComparisonOperator.EQUAL,
  "equal comparison",
);
export const BUY_ACTION: GraphBrokerActionDescriptor = requireDescriptor(
  TEST_GRAPH_CATALOG.broker_actions,
  (descriptor) => descriptor.action === catalogContract.graphBrokerAction.SUBMIT_BUY,
  "buy action",
);
export const SELL_ACTION: GraphBrokerActionDescriptor = requireDescriptor(
  TEST_GRAPH_CATALOG.broker_actions,
  (descriptor) => descriptor.action === catalogContract.graphBrokerAction.SUBMIT_SELL,
  "sell action",
);

export function graphFieldHandle(trigger: GraphTriggerDescriptor, path: string): string {
  const field = trigger.payload?.fields.find(
    (candidate) => candidate.path.segments.join(catalogContract.graphFieldPathSeparator) === path,
  );
