<script lang="ts">
  ...
  let missingMetadataSlugs = $state<string[]>([]);
  ...
  $effect(() => {
    ...
    lookupRetry;
    const lookupSlugs = untrack(() => slugs.filter((slug) => !metadata[slug]));
    lookupError = false;
    if (lookupSlugs.length === 0) return;
    const controller = new AbortController();
    void (async () => {
      try {
        const { data } = await lookupMarkets({
          body: { slugs: lookupSlugs },
          signal: controller.signal,
          throwOnError: true,
        });
        if (controller.signal.aborted) return;
...
>
    ...
    <div class="suggestions">
      <div class="search-status" role="status" aria-live="polite">
        ...
        {:else if searched && results.length === 0}
          {MARKET_SELECTOR_COPY.NO_RESULTS}
  ...
  {#if value.length}
    <div class="selection-heading">
      <span>Selected markets</span><span>{value.length} / {limits.maximumSelections}</span>
    </div>
      ...
      {#each value as slug (slug)}
        ...
        <li>
          ...
          <button
            type="button"
            class="remove-market"
            aria-label={`Remove ${market?.question ?? slug}`}
            {disabled}
            onclick={() => onchange(value.filter((item) => item !== slug))}><X size={17} /></button
          >
        </li>
      {/each}
    </ul>
  ...
  {#if lookupError}
    <p class="lookup-error" role="status">
      {MARKET_SELECTOR_COPY.LOOKUP_ERROR}
      <button type="button" class="text-button" onclick={() => (lookupRetry += 1)}
        >{MARKET_SELECTOR_COPY.RETRY_DETAILS}</button
      >
    </p>
  {/if}
</div>
