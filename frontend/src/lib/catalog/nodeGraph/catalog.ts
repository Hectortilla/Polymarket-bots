...
export function triggerForNode(catalog: GraphNodeCatalog, nodeData: GraphTriggerNodeData): GraphTriggerDescriptor {
...
export function constantForNode(catalog: GraphNodeCatalog, nodeData: GraphConstantNodeData): GraphConstantDescriptor {
  const descriptor = catalog.constants.find((candidate) => candidate.scalar_type === nodeData.scalar_type);
...
export function comparisonForNode(
  ...
  const descriptor = catalog.comparisons.find((candidate) => candidate.operator === nodeData.operator);
...
export function brokerActionForNode(
  ...
  const descriptor = catalog.broker_actions.find((candidate) => candidate.action === nodeData.action);
