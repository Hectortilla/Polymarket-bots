...
describe('decision preview', () => {
  it('renders real backend-shaped results and submits exact string cash', async () => {
    ...
    expect(
      JSON.parse((screen.getByLabelText('Sample positions') as HTMLTextAreaElement).value),
    ...
    await fireEvent.click(
      screen.getByRole('button', { name: GRAPH_PREVIEW_COPY.PREVIEW, hidden: true }),
    );
    await waitFor(() => expect(previewGraph).toHaveBeenCalled());
    expect(vi.mocked(previewGraph).mock.calls[0][0].body?.portfolio?.available_cash).toBe(
      '9007199254740993.01',
    );
