...
function descriptor(overrides: Partial<BotDefinitionDescriptor> = {}): BotDefinitionDescriptor {
  return {
...
describe("LaunchForm", () => {
  ...
  it("rejects wallet addresses that violate the generated selector contract", async () => {
    ...
    expect(screen.getByLabelText("Wallet addresses").getAttribute("aria-invalid")).toBe("true");
  });
  ...
  it("renders another supported catalog schema without definition-specific code", async () => {
    ...
    } as Awaited<ReturnType<typeof searchMarkets>>);
    const submit = vi.fn();
  ...
  it("shows schema feedback and does not submit malformed JSON", async () => {
    ...
    expect(streamRules.getAttribute("aria-describedby")).toBe("field-stream_rules-error");
  ...
  it("hydrates saved inputs, reports edits, and can block running while dirty", async () => {
    ...
    expect((screen.getByLabelText("Name") as HTMLInputElement).value).toBe("Saved bot");
    expect((screen.getByLabelText("Max order size") as HTMLInputElement).value).toBe("2.500");
    expect(screen.getByRole<HTMLButtonElement>("button", { name: "Run bot" }).disabled).toBe(true);
