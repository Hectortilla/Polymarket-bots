...
describe('account return paths', () => {
  ...
  it.each(['https://foreign.example/', '//foreign.example/', '/\\foreign.example', '/login', '/register', '/\n/foreign.example', 'javascript:alert(1)'])('rejects unsafe or cyclic return path %s', value => {
