  ...
  async function submit(event: SubmitEvent): Promise<void> {
    ...
    try {
      ...
      if (result.data) message = ACCOUNT_COPY.SENT;
        ...
        error =
          result.response?.status === HTTP_STATUS.TOO_MANY_REQUESTS
            ? AUTH_COPY.RATE_LIMIT_ERROR
            : ACCOUNT_COPY.DELIVERY_FAILED;
    } catch {
      error = ACCOUNT_COPY.DELIVERY_FAILED;
    } finally {
      busy = false;
    }
  }
...
<form onsubmit={submit}>
  <label
    >{AUTH_COPY.EMAIL}<input
      type="email"
      autocomplete="email"
      maxlength={contract.auth.emailMaxLength}
      bind:value={email}
      required
    /></label
  >
