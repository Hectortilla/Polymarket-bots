...
function validate(url: string, body: unknown) {
  return validateOperationResponse(
    ...
    new Request("https://control-plane.test"),
    { url },
  );
}
...
describe("account operation responses", () => {
  it.each(actionPaths)("requires exact acceptance for %s", async (path) => {
    ...
    for (const body of [null, {}, { accepted: false }, { accepted: "true" }, { accepted: true, token: "unexpected" }]) {
  ...
  it("requires both account status flags without extra fields", async () => {
    for (const email_verified of [true, false]) {
      for (const verification_required of [true, false]) {
        await expect(validate(paths.accountStatus, { email_verified, verification_required })).resolves.toBeInstanceOf(
          Response,
        );
      }
    }
...
describe("account status and action copy", () => {
  ...
  it("distinguishes required verification from retained legacy access", () => {
    expect(accountStatusMessage({ email_verified: false, verification_required: true })).toBe(
      ACCOUNT_COPY.VERIFICATION_REQUIRED,
    );
    expect(accountStatusMessage({ email_verified: false, verification_required: false })).toBe(
      ACCOUNT_COPY.LEGACY_ACCOUNT,
    );
  });
  it.each([undefined, HTTP_STATUS.INTERNAL_SERVER_ERROR, HTTP_STATUS.SERVICE_UNAVAILABLE])(
    "shows service failure for %s",
    (status) => {
