<script lang="ts">
  ...
  async function submit(event: SubmitEvent): Promise<void> {
    ...
    error = "";
    try {
      const outcome = await submitCredentials(registering ? ACCOUNT_ACTION.REGISTER : ACCOUNT_ACTION.LOGIN, {
        email,
        password,
      });
      if (outcome === CREDENTIAL_OUTCOME.AUTHENTICATED) {
        accountSession.acceptLogin(
          registering
            ? runtimeContract.accountManagement.accountPath
            : page.url.searchParams.get(RETURN_TO_QUERY_PARAM),
        );
...
<section class="account-panel">
  ...
  <p>
    {registering
      ...
      : "Sign in to your bots, graphs, and run history."}
  </p>
  ...
  <p>
    {registering ? "Already have an account?" : `New to ${SERVICE_NAME}?`}
    <a href={accountPath(registering ? LOGIN_PATH : REGISTER_PATH, page.url.searchParams.get(RETURN_TO_QUERY_PARAM))}
      >{registering ? AUTH_COPY.SIGN_IN : AUTH_COPY.REGISTER}</a
    >
  </p>
</section>
