...
afterEach(() => { session.clear(); vi.clearAllMocks(); vi.unstubAllGlobals(); session = new AccountSession(); });
...
describe('private account lifetime', () => {
  ...
  it('discards a restoration that completes after logout', async () => {
    ...
    mocks.currentUser.mockReturnValue(new Promise(done => { resolve = done; }));
...
describe('account failure and race boundaries', () => {
  ...
  it('does not restore private data while revocation is pending', async () => {
    let revoke!: () => void;
    mocks.logout.mockReturnValue(new Promise<void>(resolve => { revoke = resolve; }));
  ...
  it('does not let stale restoration end an in-progress logout', async () => {
    ...
    let revoke!: () => void;
    mocks.currentUser.mockReturnValue(new Promise(resolve => { restore = resolve; }));
    mocks.logout.mockReturnValue(new Promise<void>(resolve => { revoke = resolve; }));
    const replace = vi.fn();
    vi.stubGlobal('window', { location: { pathname: runPath('example'), search: '', replace } });
  ...
  it('validates cross-tab messages and closes streams before reloading', () => {
    class FakeChannel {
      ...
      close = vi.fn();
      postMessage = vi.fn();
      constructor(readonly name: string) { FakeChannel.current = this; }
    }
    const reload = vi.fn();
