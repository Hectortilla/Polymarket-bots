...
export const CREDENTIAL_OUTCOME = { AUTHENTICATED: 'authenticated', REJECTED: 'rejected', RATE_LIMITED: 'rate-limited', UNAVAILABLE: 'unavailable' } as const;
...
export async function submitCredentials(
  action: AccountAction, credentials: CredentialsWritable,
  ...
  try {
    ...
    if (!result.response || result.response.status >= HTTP_STATUS.INTERNAL_SERVER_ERROR) return CREDENTIAL_OUTCOME.UNAVAILABLE;
