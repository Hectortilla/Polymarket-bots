...
const USAGE_COUNT_FIELDS = ['active_runs', 'queued_runs', 'saved_bots', 'saved_templates', 'retained_runs'] as const satisfies readonly (keyof AccountUsage)[];
...
export function isAccountUsage(value: unknown): value is AccountUsage {
  ...
  return Object.keys(runtimeContract.resourcePolicy).every(
    (key) => Number.isSafeInteger(policy[key]) && (policy[key] as number) > 0,
  ) && USAGE_COUNT_FIELDS.every(
