...
export function isOneOf<Value extends string>(value: unknown, choices: readonly Value[]): value is Value {
  return typeof value === "string" && choices.includes(value as Value);
}

export function isArrayOf(data: unknown, predicate: (value: Record<string, unknown>) => boolean): boolean {
...
export function isOptionalNullable(value: unknown, predicate: (candidate: unknown) => boolean): boolean {
