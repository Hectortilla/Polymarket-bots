...
vi.mock('$lib/api/generated', () => ({ createGraphTemplateApiV1GraphTemplatesPost: mocks.template, createBotApiV1BotsPost: mocks.bot }));
...
it('reuses confirmed template after bot failure and confirmed bot after navigation failure', async () => {
  ...
  mocks.bot.mockRejectedValueOnce({ code: contract.resourceLimitCodes.USER_ALLOWANCE, detail: 'capacity' }).mockResolvedValue({ data: { id: 'saved-bot' } });
  const draft = new SavedBotDraft();
  await expect(draft.save('example-definition', { name: 'First' }, TEST_GRAPH)).rejects.toMatchObject({ uncertain: false });
...
it.each(Object.values(DRAFT_WRITE).flatMap(stage => [HTTP_STATUS.INTERNAL_SERVER_ERROR, undefined].map(status => ({ stage, status }))))('fences a resolved uncertain $stage write (status=$status)', async ({ stage, status }) => {
  mocks.template.mockResolvedValue({ data: { id: 'private-template' } });
  mocks.bot.mockResolvedValue({ data: { id: 'saved-bot' } });
  mocks[stage].mockResolvedValueOnce({ error: new TypeError('fetch failed'), response: status === undefined ? undefined : new Response(null, { status }) });
  const draft = new SavedBotDraft();
  await expect(draft.save('example-definition', {}, TEST_GRAPH)).rejects.toMatchObject({ uncertain: true });
  await expect(draft.save('example-definition', {}, TEST_GRAPH)).rejects.toMatchObject({ uncertain: true });
  expect(mocks[stage]).toHaveBeenCalledTimes(1);
