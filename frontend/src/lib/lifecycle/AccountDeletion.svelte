...
<section aria-labelledby="delete-account-heading">
  ...
  <p>This immediately signs out every session and stops queued and active paper runs. Eligible account details, bots, graphs and run history are removed within {contract.dataLifecycle.deletionTargetHours} hours. You cannot undo the request.</p>
  <p>Encrypted backups expire within {contract.dataLifecycle.backupRetentionDays} days. Minimal deletion and operator receipts remain for {contract.dataLifecycle.auditRetentionDays} days. Export any graphs you need before continuing.</p>
  <form onsubmit={submit}>
    <label>{LIFECYCLE_COPY.PASSWORD}<input type="password" autocomplete="current-password" minlength={contract.auth.passwordMinLength} maxlength={contract.auth.passwordMaxLength} bind:value={password} required /></label>
