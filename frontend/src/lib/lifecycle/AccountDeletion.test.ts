...
afterEach(cleanup);
beforeEach(() => { request.mockReset(); revoked.mockReset(); });

async function confirmedForm() {
  render(AccountDeletion);
  await fireEvent.input(screen.getByLabelText(LIFECYCLE_COPY.PASSWORD), { target: { value: 'deletion fixture password' } });
