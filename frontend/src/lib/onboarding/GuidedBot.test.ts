...
const mocks = vi.hoisted(() => ({ definitions: vi.fn(), template: vi.fn(), bot: vi.fn(), usage: vi.fn(), goto: vi.fn() }));
vi.mock('$app/navigation', () => ({ goto: mocks.goto }));
vi.mock('$lib/api/generated', () => ({ listBotDefinitionsApiV1BotDefinitionsGet: mocks.definitions, createGraphTemplateApiV1GraphTemplatesPost: mocks.template, createBotApiV1BotsPost: mocks.bot, readUsage: mocks.usage }));
import GuidedBot from './GuidedBot.svelte';

const definition = {
  definition_id: 'test-example', display_name: 'Test graph', description: 'A fixture',
  label: BOT_DEFINITION_LABEL.STANDARD, market_selection: SELECTION_MODE.USER_CONFIGURED,
  wallet_selection: SELECTION_MODE.ABSENT, graph_catalog: TEST_GRAPH_CATALOG, starter_graph: TEST_GRAPH,
...
beforeEach(() => {
  ...
  mocks.usage.mockResolvedValue({ data: { policy: contract.resourcePolicy, active_runs: 0, queued_runs: 0, saved_bots: 0, saved_templates: 0, retained_runs: 0 } });
...
it('loads examples with retry and preserves a route back to existing bots', async () => {
  ...
  expect(screen.getByRole('link', { name: NAVIGATION_LABEL.BACK_TO_BOTS })).toHaveAttribute('href', NAVIGATION_PATH.HOME);
...
it('does not persist before review, retains settings on Back, and saves without launch', async () => {
  ...
  expect(mocks.bot).toHaveBeenCalledWith(expect.objectContaining({ body: { definition_id: definition.definition_id, inputs: { name: 'My example' }, graph_template_id: 'private-template' } }));
});
it('retains the review after capacity rejection and reuses the confirmed template', async () => {
  mocks.bot.mockRejectedValueOnce({ code: Object.values(contract.resourceLimitCodes)[0], detail: RESOURCE_LIMIT_DETAIL });
  render(GuidedBot);
  await review();
  await fireEvent.click(screen.getByRole('button', { name: ONBOARDING_COPY.SAVE }));
...
it('returns authoritative field errors to settings without losing entered values', async () => {
  mocks.bot.mockResolvedValueOnce({ error: { detail: [{ loc: ['body', 'inputs', 'name'], msg: 'Name was rejected', type: 'value_error' }] }, response: new Response(null, { status: HTTP_STATUS.UNPROCESSABLE_CONTENT }) });
  render(GuidedBot); await review();
  await fireEvent.click(screen.getByRole('button', { name: ONBOARDING_COPY.SAVE }));
...
it.each([{ definitions: [] }, { definitions: [{ ...definition, graph_examples: [] }] }])('rejects an unusable successful catalog response', async ({ definitions }) => {
...
it('reviews and saves the selected second example', async () => {
  const second = { name: 'Example two', description: 'A second set of conditions', graph: { ...TEST_GRAPH, parameters: [] } };
  mocks.definitions.mockResolvedValue({ data: [{ ...definition, graph_examples: [...definition.graph_examples, second] }] });
  render(GuidedBot);
  ...
  await fireEvent.click(screen.getByRole('button', { name: ONBOARDING_COPY.SAVE }));
  await waitFor(() => expect(mocks.template).toHaveBeenCalledWith(expect.objectContaining({ body: expect.objectContaining({ graph: second.graph }) })));
});

it('renders a template graph rejection and retries without writing a bot first', async () => {
  mocks.template.mockResolvedValueOnce({ error: { detail: [{ loc: ['body', 'graph'], msg: 'Graph input cardinality is invalid', type: 'value_error' }] }, response: new Response(null, { status: HTTP_STATUS.UNPROCESSABLE_CONTENT }) });
  render(GuidedBot); await review();
  await fireEvent.click(screen.getByRole('button', { name: ONBOARDING_COPY.SAVE }));
