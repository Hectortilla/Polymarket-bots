<script lang="ts">
  ...
  let { descriptor, example, inputs }: { descriptor: BotDefinitionDescriptor; example: GraphExample; inputs: LaunchInputs } = $props();
</script>
<h3>{example.name}</h3>
...
<dl>
  {#each example.graph.parameters ?? [] as parameter (parameter.id)}
    <div><dt>{parameter.name}</dt><dd>{String(parameter.data.value)}</dd></div>
  {/each}
</dl>
<h3>{ONBOARDING_COPY.SETTINGS}</h3>
<dl>
  {#each launchFields(descriptor) as [name, schema] (name)}
    <div><dt>{fieldLabel(name, resolvedFieldSchema(descriptor, schema))}</dt><dd>{Array.isArray(inputs[name]) ? inputs[name].join(', ') : String(inputs[name] ?? '')}</dd></div>
  {/each}
</dl>
...
<style>
  ...
  dl div { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 1rem; }
  dt, dd { margin: 0; overflow-wrap: anywhere; }
  @media (max-width: 560px) { dl div { grid-template-columns: 1fr; gap: .2rem; } }
</style>
