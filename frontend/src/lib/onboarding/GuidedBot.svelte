<script lang="ts">
  ...
  onMount(() => { void loadOnboardingExamples(); });
  ...
  async function loadOnboardingExamples(): Promise<void> {
    ...
    } catch { error = ONBOARDING_COPY.LOAD_ERROR; }
...
<section class="guided-bot">
  ...
  {:else}
    ...
    <div hidden={step !== ONBOARDING_STEP.EXAMPLE}>
      <fieldset disabled={saving}><legend>{ONBOARDING_COPY.CHOOSE}</legend>
        {#each descriptor.graph_examples ?? [] as option, index}
          <label><input type="radio" name="example" value={index} bind:group={exampleIndex} /> <strong>{option.name}</strong><span>{option.description}</span></label>
        {/each}
      </fieldset>
      <p>{ONBOARDING_COPY.PARAMETERS}: {(example.graph.parameters ?? []).map(parameter => `${parameter.name}: ${parameter.data.value}`).join('; ')}</p>
    ...
    <div hidden={step !== ONBOARDING_STEP.SETTINGS}>
      <LaunchForm {descriptor} onsubmit={review} busy={saving} serverIssues={inputIssues}
        submitLabel={ONBOARDING_COPY.REVIEW} showSelectionNotes={false}
    ...
    <div hidden={step !== ONBOARDING_STEP.REVIEW}>
      ...
      <div class="actions"><button onclick={savePrivateBot} disabled={saving || saveUncertain}>{saving ? ONBOARDING_COPY.SAVING : ONBOARDING_COPY.SAVE}</button>
      <button class="secondary" disabled={saving} onclick={() => moveToStep(ONBOARDING_STEP.SETTINGS)}>{ONBOARDING_COPY.BACK}</button></div>
    </div>
  {/if}
  <p><a href={NAVIGATION_PATH.NEW_BOT}>{ONBOARDING_COPY.ADVANCED}</a></p>
</section>
<style>
  .guided-bot { max-width: 840px; margin: 0 auto; padding-block: 2rem; }
  fieldset { border: 0; padding: 0; display: grid; gap: 1rem; margin-bottom: 1.5rem; }
  label { display: grid; grid-template-columns: auto 1fr; gap: .4rem .75rem; padding-block: .75rem; }
  label span { grid-column: 2; }
  ol { padding-left: 1.2rem; display: flex; flex-wrap: wrap; gap: 1rem 2rem; }
  [aria-current="step"] { font-weight: 700; }
