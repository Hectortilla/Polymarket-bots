...
afterEach(() => { cleanup(); SERVICE_IDENTITY.supportEmail = originalContact; });
...
it('shows a configured mailbox and removes the placeholder state', () => {
  ...
  render(SupportContact);
  expect(screen.getByRole('link', { name: SERVICE_IDENTITY.supportEmail })).toHaveAttribute('href', `mailto:${SERVICE_IDENTITY.supportEmail}`);
