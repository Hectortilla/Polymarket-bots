...
export function requestValidationIssues(error: unknown): RequestValidationIssue[] {
...
export function readableValidationMessage(message: string): string {
  ...
  const sentence = withoutPydanticPrefix.replace(/^./, (letter) => letter.toUpperCase());
...
function isValidationIssue(value: unknown): value is ValidationError {
  ...
  return Boolean(
    ...
    issue.loc.every((segment) => typeof segment === "string" || typeof segment === "number") &&
