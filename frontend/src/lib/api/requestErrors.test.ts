...
import { readableValidationMessage, requestErrorDetail, requestValidationIssues } from "./requestErrors";
...
describe("request error presentation", () => {
  it("keeps only valid FastAPI validation issues", () => {
    ...
    expect(requestValidationIssues({ detail: [validIssue, { msg: "missing location" }] })).toEqual([validIssue]);
  });
  ...
  it("removes transport wording and returns a complete sentence", () => {
    expect(readableValidationMessage("Value error, graph input cardinality is invalid")).toBe(
      "Graph input cardinality is invalid.",
    );
  });
});
