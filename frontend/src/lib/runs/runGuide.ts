...
export const RUN_GUIDE_COPY = {
  ...
  FAILED: 'This run failed or was interrupted. Read its failure detail, check the saved settings, and start a new run only when ready.',
  RECONNECTING: 'The browser is reconnecting. Displayed results may be behind; wait for reconnection or reload to recover retained history.',
  UNAVAILABLE: 'Market data is stale or has not been reported. No orders here does not prove that strategy conditions are unmet. Check stream health before continuing.',
  WAITING: 'No orders in the loaded history. The latest report shows book input received without a stale flag; the example may be waiting for its conditions.',
  ACTIONS: 'Orders appear in the loaded history. Compare each order with its fill or rejection; an order request alone does not mean a fill.',
  BALANCES: 'Paper cash, positions, fees and equity are simulated. Equity can be unavailable when a position cannot be valued; unavailable is not zero.',
  EVENTS: 'Progress shows orders, fills, and available skipped/rejected-action reasons. A disabled condition can produce no action or progress entry. Loaded counts cover the visible history window.',
} as const;

export function runGuidance(status: RunStatus, health: StreamHealthPayload | null, reconnecting: boolean, loadedOrderCount: number): string {
...
export function hasReportedUsableBook(health: StreamHealthPayload | null): boolean {
  return health !== null && !health.book_stale && health.book_received_count > 0 && health.book_dispatch_lag_ms !== null;
}
