...
import { isActivityPayload, isBootstrapPayload, isLifecyclePayload } from "$lib/runs/eventPayloads/lifecycle";
...
export function isDurableEventPayload(kind: PersistedDurableEvent["kind"], payload: Record<string, unknown>): boolean {
  switch (kind) {
...
function isWalletTradeKind(payload: Record<string, unknown>): boolean {
  return (
    isRecord(payload.trade) && (payload.trade.kind === undefined || isOneOf(payload.trade.kind, WALLET_TRADE_KINDS))
  );
}
