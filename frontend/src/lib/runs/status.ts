...
type RunStatusContract = {
  [Key in keyof typeof runtimeContract.runStatus.values]: Extract<RunStatus, Lowercase<Key>>;
};
export const RUN_STATUS = runtimeContract.runStatus.values as RunStatusContract;
const TERMINAL_RUN_STATUSES = new Set<RunStatus>(runtimeContract.runStatus.terminal as RunStatus[]);
const STOPPABLE_RUN_STATUSES = new Set<RunStatus>(runtimeContract.runStatus.stoppable as RunStatus[]);
