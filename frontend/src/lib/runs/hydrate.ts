...
export async function loadOlderRunEvents(runId: string, beforeEventId: number): Promise<PersistedEventPage> {
...
export async function loadAndContinueRunDetail(
  ...
  return openStream(hydration.run.id, hydration.cursor, onDurableEvent, onLiveEvent, onConnectionState);
}
