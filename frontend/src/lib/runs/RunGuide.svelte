<script lang="ts">
  ...
  let { run, events, health, reconnecting }: { run: RunRead; events: PersistedDurableEvent[]; health: StreamHealthPayload | null; reconnecting: boolean } = $props();
...
<style>.run-guide { border-block: 1px solid var(--line); padding-block: 1rem; margin-block: 1.5rem; }</style>
