...
const health: StreamHealthPayload = { queue_depth: 0, peak_queue_depth: 0, book_dispatch_lag_ms: 0, book_stale: false, book_received_count: 5, book_coalesced_count: 0 };
describe('run-reading guidance', () => {
  it.each([
    [RUN_STATUS.QUEUED, RUN_GUIDE_COPY.QUEUED], [RUN_STATUS.STARTING, RUN_GUIDE_COPY.STARTING],
    [RUN_STATUS.STOP_REQUESTED, RUN_GUIDE_COPY.STOPPING], [RUN_STATUS.STOPPING, RUN_GUIDE_COPY.STOPPING],
    [RUN_STATUS.STOPPED, RUN_GUIDE_COPY.STOPPED], [RUN_STATUS.FAILED, RUN_GUIDE_COPY.FAILED],
    [RUN_STATUS.INTERRUPTED, RUN_GUIDE_COPY.FAILED],
  ])('explains %s', (status, copy) => expect(runGuidance(status, null, false, 0)).toBe(copy));
  it.each([null, { ...health, book_stale: true }, { ...health, book_received_count: 0 }, { ...health, book_dispatch_lag_ms: null }])('does not infer waiting from unavailable data', (sample) => {
