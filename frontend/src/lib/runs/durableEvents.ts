...
export function persistedDurableEvent(value: unknown, runId: string): PersistedDurableEvent | null {
  ...
  if (event.kind === EVENT_KIND.runLifecycle && event.payload.status === undefined) {
    return {
      ...event,
      id: Number(event.id),
...
export function requirePersistedEventPage(page: RunEventPage, runId: string): PersistedEventPage {
...
export function isTerminalLifecycleEvent(event: PersistedDurableEvent): boolean {
  return event.kind === EVENT_KIND.runLifecycle && isTerminalRunStatus(event.payload.status);
}
