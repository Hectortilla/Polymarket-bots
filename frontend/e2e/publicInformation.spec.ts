...
import { PUBLIC_INFORMATION_PATH, PUBLIC_INFORMATION_LABEL, PUBLIC_INFORMATION_NAV_LABEL } from '../src/lib/public/navigation';
...
test('anonymous information navigation never requests private resources and account links remain usable', async ({ page }) => {
  ...
  for (const path of Object.values(PUBLIC_INFORMATION_PATH)) {
    const link = page.getByRole('navigation', { name: PUBLIC_INFORMATION_NAV_LABEL }).getByRole('link', { name: PUBLIC_INFORMATION_LABEL[path], exact: true });
    await link.hover();
    await link.click();
...
test('authenticated information visits suppress session polling and resume it on private navigation', async ({ page }) => {
  ...
  await page.getByRole('link', { name: PUBLIC_INFORMATION_LABEL[PUBLIC_INFORMATION_PATH.SUPPORT], exact: true }).click();
  await expect(page).toHaveURL(url => url.pathname === PUBLIC_INFORMATION_PATH.SUPPORT);
  const privateRequests: string[] = [];
  page.on('request', request => { if (['fetch', 'xhr'].includes(request.resourceType())) privateRequests.push(request.url()); });
  ...
  await expect.poll(() => privateRequests.some(url => new URL(url).pathname === contract.apiPaths.currentUser)).toBe(true);
...
test('public claims and links reflect paper-only operation and the generated data policy', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: AUTH_COPY.REGISTER, exact: true })).toHaveAttribute('href', REGISTER_PATH);
  ...
  await expect(page.getByRole('link', { name: PUBLIC_COPY.RESET, exact: true })).toHaveAttribute('href', contract.accountManagement.forgotPath);
  ...
  await page.goto(PUBLIC_INFORMATION_PATH.SUPPORT);
  await expect(page.getByRole('status')).toHaveText(SUPPORT_PENDING);
  await expect(page.getByText(PUBLIC_COPY.RESPONSIBLE_OPERATOR, { exact: false })).toContainText(SERVICE_IDENTITY.operator);
  await expect(page.getByRole('link', { name: PUBLIC_COPY.RECOVER })).toHaveAttribute('href', contract.accountManagement.forgotPath);
});
