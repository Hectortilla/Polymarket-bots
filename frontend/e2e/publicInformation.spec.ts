...
import { SERVICE_IDENTITY } from "../src/lib/public/identity";
...
test("anonymous information navigation only checks the session and account links remain usable", async ({ page }) => {
...
test("the shared header keeps account controls on public pages without private polling and supports sign out", async ({
  ...
  await page.getByRole("link", { name: PUBLIC_INFORMATION_LABEL[PUBLIC_INFORMATION_PATH.HELP], exact: true }).click();
...
test("public claims and links reflect paper-only operation and the generated data policy", async ({ page }) => {
  ...
  await page.goto(PUBLIC_INFORMATION_PATH.SUPPORT);
  await expect(page.getByRole("link", { name: SERVICE_IDENTITY.supportEmail! })).toHaveAttribute(
    "href",
...
test("overview and help keep signed-in guidance after a direct visit or reload", async ({ page }) => {
  await page.goto(REGISTER_PATH);
  const email = `returning-reader-${Date.now()}@example.com`;
  await page.getByLabel(AUTH_COPY.EMAIL, { exact: true }).fill(email);
  await page.getByLabel(AUTH_COPY.PASSWORD, { exact: true }).fill("returning reader password 123");
  await page.getByRole("button", { name: AUTH_COPY.REGISTER, exact: true }).click();
  await expect(page).toHaveURL(contract.accountManagement.accountPath);

  for (const path of [PUBLIC_INFORMATION_PATH.WELCOME, PUBLIC_INFORMATION_PATH.HELP]) {
    await page.goto(path);
    await expect(page.getByRole("banner").getByText(email, { exact: true })).toBeVisible();
    await expect(page.getByRole("main").getByRole("link", { name: AUTH_COPY.REGISTER, exact: true })).toHaveCount(0);
    if (path === PUBLIC_INFORMATION_PATH.WELCOME) {
      await expect(page.getByRole("link", { name: "Your bots", exact: true })).toHaveAttribute(
        "href",
        NAVIGATION_PATH.HOME,
      );
    } else {
      await expect(page.getByRole("heading", { name: "Start a run", exact: true })).toBeVisible();
      await expect(
        page.getByRole("main").getByRole("link", { name: ACCOUNT_COPY.SETTINGS, exact: true }),
      ).toBeVisible();
    }
    await page.reload();
    await expect(page.getByRole("banner").getByText(email, { exact: true })).toBeVisible();
    await expect(page.getByRole("main").getByRole("link", { name: AUTH_COPY.SIGN_IN, exact: true })).toHaveCount(0);
  }
});

test("public content stays readable if the session check is unavailable", async ({ page }) => {
  await page.route(`**${contract.apiPaths.currentUser}`, (route) =>
    route.fulfill({ status: HTTP_STATUS.SERVICE_UNAVAILABLE, json: {} }),
  );
  await page.goto(PUBLIC_INFORMATION_PATH.HELP);
  await expect(page.getByRole("heading", { name: "Help", exact: true })).toBeVisible();
  await expect(page.getByRole("alert")).toHaveText(AUTH_COPY.RESTORE_ERROR);
  await expect(page).toHaveURL(PUBLIC_INFORMATION_PATH.HELP);
});
