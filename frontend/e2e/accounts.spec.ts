...
test("two accounts keep editor, copies, runs and history private across reload and switching", async ({
  page,
  browser,
}) => {
  const privateRequests: string[] = [];
  page.on("request", (request) => {
    if ([contract.apiPaths.bots, contract.apiPaths.runs].some((path) => request.url().endsWith(path)))
...
test("active streams close on logout, external revocation, and cross-tab account switching", async ({
  page,
  browser,
}) => {
...
test("lost launch response and reload recover one private run", async ({ page }) => {
  ...
  expect(
    (await history.json()).filter((run: { bot_id: string }) => run.bot_id === botUrl.split("/").pop()),
  ).toHaveLength(1);
  await page.getByRole("button", { name: RUN_STATUS_PRESENTATION[RUN_STATUS.QUEUED].stopLabel!, exact: true }).click();
  await expect(
    page.getByText(RUN_STATUS_PRESENTATION[RUN_STATUS.STOPPED].label, { exact: true }).first(),
