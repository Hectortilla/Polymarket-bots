...
export async function readAccountLink(page: Page, email: string): Promise<string> {
...
export async function completeEmailVerification(page: Page, email: string, password: string): Promise<void> {
  ...
  await page.goto(await readAccountLink(page, email));
