...
for (const scenario of Object.keys(expectations) as (keyof typeof expectations)[]) {
  ...
  test(`guided ${scenario} paper run saves first, explains results and stops`, async ({ page, request }) => {
    ...
    if (expected.recoverSearch) {
      await page.route(`**${contract.apiPaths.marketSearch}*`, route => route.fulfill({ status: HTTP_STATUS.SERVICE_UNAVAILABLE, body: '{}' }), { times: 1 });
    ...
    await page.getByRole('option').filter({ hasText: `Will ${marketSlug} happen?` }).click();
    ...
    await page.getByRole('button', { name: RUN_STATUS_PRESENTATION[RUN_STATUS.RUNNING].stopLabel!, exact: true }).click();
