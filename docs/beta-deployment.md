...
| Bootstrap credentials fail with `no_log: true` and censored output | Rerun with the updated bootstrap. `Require valid bootstrap credentials before host mutation` now reports a sanitized field/check diagnostic while the credential-bearing command retains `no_log`. Correct the named input locally using `ansible-vault edit` (or the public inventory for the SSH public key), then rerun the full bootstrap. It distinguishes malformed keys, SFTP host-record mismatches, invalid age recipients, missing controller tools, SMTP credentials, Tailscale auth-key prefixes and Cloudflare tokens. Raw tool output and secret values remain hidden. |
| Bootstrap preflight reports that the controller could not run | Check `uv` is available and run `uv sync --extra dev` from the repository root. OpenSSH `ssh-keygen` is required locally; `age` is also required when backups are enabled. The task fails before host mutation even when no structured diagnostic is available. |
| Deprecation warning for `play_hosts` at an inventory or bootstrap `stdin` expression | The earlier `vars \| dict2items` expression evaluated unrelated Ansible magic variables before filtering. The updated task selects `polybot_` names first, then reuses the normalized inventory. This warning is separate from a failed credential check. |
